export const Specialty = () => {
  return (
<>
<div className="specialty section container" id="specialty">
                <div className="specialty__container">
                    <div className="specialty__box">
                        <h2 className="section__title">
                            Specialty coffees and teas from china and cheer you up!
                        </h2>
                        <div>
                            <a href="#" className="button specialty__button">See more</a>
                        </div>
                    </div>
                    <div className="specialty__category">
                        <div className="specialty__group">
                            <img src="src/img/specialty1.png" alt="" className="specialty__img"/>
                            <h3 className="specialty__title">goku</h3>
                            <p className="specialty__description">
                            Specialty coffees and teas from china and cheer you up!
                            </p>
                        </div>
                        <div className="specialty__group">
                            <img src="src/img/specialty2.png" alt="" className="specialty__img"/>
                            <h3 className="specialty__title">goku</h3>
                            <p className="specialty__description">
                            Specialty coffees and teas from china and cheer you up!
                            </p>
                        </div>
                        <div className="specialty__group">
                            <img src="src/img/specialty3.png" alt="" className="specialty__img"/>
                            <h3 className="specialty__title">goku</h3>
                            <p className="specialty__description">
                            Specialty coffees and teas from china and cheer you up!
                            </p>
                        </div>
                    </div>
                </div>
            </div>
</>  )
}
